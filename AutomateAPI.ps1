...
function Get-AutomateControlGUID {
    ...
    $url = ($Global:CWAUri + "/v1/extensionactions/control/$ComputerID")
...
function Get-AutomateComputer {
    ...
    Get-AutomateAPIOutputGeneric -AllResults -APIURI "/v1/computers?" -Condition $FinalCondition
}
...
function Get-AutomateControlReconcile {
    ...
    process {
        ...
        foreach ($CompToCheck in $ComputersToCheck) {
            ...
            $ControlGuid = Get-AutomateControlGUID -ComputerID $CompToCheck.ID
