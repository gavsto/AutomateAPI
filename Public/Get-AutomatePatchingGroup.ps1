function Get-AutomatePatchingGroup {
    <#
    .SYNOPSIS
        Get Group Patching information out of the Automate API
    .DESCRIPTION
        Connects to the Automate API and returns one or more full Group Patching objects
    .PARAMETER AllGroups
    ...
    .PARAMETER GroupName
        Group name to search for, uses wildcards so full Patching Group name is not needed
    .PARAMETER GroupID
    ...
    #>
        ...
        if ($GroupID) {
            Return Get-AutomateAPIGeneric -AllResults -Endpoint "GroupPatchingPolicies" -IDs $(($GroupID) -join ",")
        }

        if ($AllGroups) {
            Return Get-AutomateAPIGeneric -AllResults -Endpoint "GroupPatchingPolicies" -IncludeFields $IncludeFields -ExcludeFields $ExcludeFields -OrderBy $OrderBy
        }

        if ($Condition) {
            Return Get-AutomateAPIGeneric -AllResults -Endpoint "GroupPatchingPolicies" -Condition $Condition -IncludeFields $IncludeFields -ExcludeFields $ExcludeFields -OrderBy $OrderBy
        }

        if ($GroupName) {
            $ArrayOfConditions += "(GroupName like '%$GroupName%')"
        }

        $GroupFinalCondition = Get-ConditionsStacked -ArrayOfConditions $ArrayOfConditions

        $Groups = Get-AutomateAPIGeneric -AllResults -Endpoint "GroupPatchingPolicies" -Condition $GroupFinalCondition -IncludeFields $IncludeFields -ExcludeFields $ExcludeFields -OrderBy $OrderBy
